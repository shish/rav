...
def toggle():
    ...
    avatar = db.one_or_404(
        db.select(Avatar).filter(Avatar.id == int(request.args["avatar_id"]))
    )
    user = db.one_or_404(db.select(User).filter(User.username == session["username"]))
...
def create():
    ...
    if password1 != password2:
        raise RavError(
            "Password Error", "The password and confirmation password don't match D:"
        )
